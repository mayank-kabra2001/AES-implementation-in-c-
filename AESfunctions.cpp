...
{
//#pragma HLS RESOURCE variable=plaintext core=RAM_1P_BRAM metadata="-bus_bundle BUS_CTRL"
//#pragma HLS INTERFACE ap_memory port=plaintext
#pragma HLS INTERFACE s_axilite port=return bundle=CRTLS
#pragma HLS INTERFACE s_axilite port=plaintext bundle=CRTLS
#pragma HLS INTERFACE s_axilite port=expandedKey bundle=CRTLS
#pragma HLS INTERFACE s_axilite port=Nr bundle=CRTLS
#pragma HLS INTERFACE s_axilite port=ciphertext bundle=CRTLS
	...
	for (unsigned short i = 0; i < stt_lng; i++) { cout << decrypted_plaintext[i] << " "; if (decrypted_plaintext[i] != plaintext[i])flag = 1;}
	cout << "<=> ";
